...
export async function GET(req: NextRequest) {
  ...
  (txns || []).forEach(t => {
    const d = DateTime.fromISO(t.posted_at as string).setZone(tz).toFormat("yyyy-LL-dd");
  ...
  for (let i = days; i >= 0; i--) {
    ...
    out.push({ date: d, starting_allowance_cents: incoming, spent_cents: spent, ending_rollover_cents: ending });
    rollover = ending;
  }
